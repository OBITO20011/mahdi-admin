...
export function StoreInfoSection({ whatsappUrl, onTrackOrder, settings }: { whatsappUrl: string; onTrackOrder: () => void; settings: PublicStorefrontSettings | null }) {
  if (!settings) {
    return <section id="store-info" aria-labelledby="store-info-heading" className="bg-white py-12">
      <div className="mx-auto max-w-7xl px-4 lg:px-8"><div className="flex flex-wrap items-end justify-between gap-3"><div><p className="text-[10px] font-black text-blue-600">قبل أن تطلب</p><h2 id="store-info-heading" className="mt-1 text-2xl font-black text-slate-950">معلومات مهمة وواضحة</h2></div><div className="flex gap-2"><button type="button" onClick={onTrackOrder} className="rounded-2xl bg-blue-700 px-4 py-3 text-xs font-black text-white">متابعة طلب</button><a href={whatsappUrl} target="_blank" rel="noreferrer" className="inline-flex items-center gap-2 rounded-2xl bg-emerald-600 px-4 py-3 text-xs font-black text-white"><Phone className="h-4 w-4" />واتساب</a></div></div>
