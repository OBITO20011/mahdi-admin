...
const runtimeSqlPath = path.join(scriptDirectory, 'final-admin-blockers-runtime.sql');
...
const runSql = (sql) => new Promise((resolve, reject) => {
  ...
  child.on('close', (code) => {
    if (code === 0) resolve(stdout.trim());
      ...
      `Final admin blocker runtime failed (exit ${code}): ${stderr.trim()}`
    ));
  });
  child.stdin.end(sql);
});
...
const summaryLine = output.split(/\r?\n/).find(
  (line) => line.startsWith('{') && line.includes('runtime_scenarios')
);
if (!summaryLine) {
  throw new Error(`Runtime did not produce a JSON summary. Output: ${output}`);
}
const summary = JSON.parse(summaryLine);
if (summary.unexpected_failures !== 0 || summary.passed !== summary.runtime_scenarios) {
  throw new Error(`Final admin blocker runtime failed: ${JSON.stringify(summary)}`);
}
